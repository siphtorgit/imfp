...
Note to maintainers: To deploy a new version of the package:

1. Increment the version number in pyproject.toml. (Follow the rules of [semantic versioning](https://semver.org/).)
2. Upgrade dependencies to their latest versions by running `uv lock --upgrade-package dependency_name` for each dependency
3. Make sure the tests still pass with `uv run pytest`
4. Make any necessary documentation changes in `NEWS` and `README.ipynb`
5. Push to dev. Github Actions will automatically format the code with black, render the README, and run the unit tests
6. If the workflow completes successfully, open a PR to main
7. After merging, the workflow will test again as one last sanity check and then automatically deploy the new version to PyPi
